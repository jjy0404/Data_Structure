...
public class MyArrayList<E> implements IList<E> {
	...
	if (this.size == this.capacity) { //expend capacity two times
	    this.capacity = this.capacity * 2;
	    Object[] TempList = new Object[this.capacity]; //copy and paste the data to new array
	    for (int i = 0; i < this.size; i++) {
		TempList[i] = this.array[i];
	    }
	    this.array = TempList;  
	}
	this.array[this.size] = element;  //add new element
	this.size++;
        return true;
    }
	...
	for (int i = 0; i < this.size; i++) {  //check evey element from the begining
